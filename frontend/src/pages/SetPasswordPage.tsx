...
export function SetPasswordPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      navigate('/login', { state: { message: 'Password set successfully. You can now sign in.' } });
    } catch (err: any) {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-dark-bg py-12 px-4">
        <div className="max-w-md w-full text-center space-y-4">
          ...
          <p className="text-text-secondary text-sm">{error || 'This link is invalid or has expired.'}</p>
          <p className="text-text-muted text-sm">Contact your administrator for a new invitation.</p>
          <Link to="/login" className="text-sm text-accent-blue hover:text-accent-blue/80 transition-colors">
            Go to Sign In
          </Link>
        </div>
      </div>
      ...
      <div className="max-w-md w-full space-y-8">
        <div className="text-center">
          ...
          <p className="mt-2 text-sm text-text-secondary">
            You've been added as <span className="text-accent-blue font-medium">{userRole.replace('_', ' ')}</span>
          </p>

...
function StateBadge({ state }: { state: AgentState }) {
  const stateConfig: Record<AgentState, { label: string; color: string; icon: React.ReactNode }> = {
    idle: { label: 'Ready', color: 'bg-dark-elevated text-text-secondary', icon: <Bot className="w-3 h-3" /> },
    analyzing: { label: 'Analyzing', color: 'bg-accent-blue/20 text-accent-blue', icon: <Loader2 className="w-3 h-3 animate-spin" /> },
    clarifying: { label: 'Needs Input', color: 'bg-accent-yellow/20 text-accent-yellow', icon: <AlertCircle className="w-3 h-3" /> },
    exploring: { label: 'Exploring', color: 'bg-accent-purple/20 text-accent-purple', icon: <Globe className="w-3 h-3 animate-pulse" /> },
    generating: { label: 'Generating', color: 'bg-accent-green/20 text-accent-green', icon: <Code className="w-3 h-3" /> },
    validating: { label: 'Validating', color: 'bg-accent-blue/20 text-accent-blue', icon: <Loader2 className="w-3 h-3 animate-spin" /> },
    reflecting: { label: 'Reflecting', color: 'bg-accent-orange/20 text-accent-orange', icon: <RefreshCw className="w-3 h-3 animate-spin" /> },
    staging: { label: 'Staging', color: 'bg-accent-purple/20 text-accent-purple', icon: <Code className="w-3 h-3" /> },
    awaiting_approval: { label: 'Review', color: 'bg-accent-yellow/20 text-accent-yellow', icon: <AlertCircle className="w-3 h-3" /> },
    merging: { label: 'Merging', color: 'bg-accent-green/20 text-accent-green', icon: <Loader2 className="w-3 h-3 animate-spin" /> },
    complete: { label: 'Complete', color: 'bg-accent-green/20 text-accent-green', icon: <Check className="w-3 h-3" /> },
    error: { label: 'Error', color: 'bg-accent-red/20 text-accent-red', icon: <X className="w-3 h-3" /> },
  };
    ...
    <div className={`flex gap-3 ${isUser ? 'flex-row-reverse' : ''}`}>
      <div
        className={`flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center ${
          isUser ? 'bg-accent-blue' : 'bg-gradient-to-br from-accent-purple to-accent-pink'
        }`}
      >
        {isUser ? (
        ...
        <div
          className={`inline-block max-w-[85%] px-4 py-2 rounded-2xl ${
            isUser
              ...
              : 'bg-dark-card text-text-primary rounded-bl-md'
          }`}
        >
...
function ClarificationBubble({
  ...
  return (
    <div className="bg-accent-yellow/10 border border-accent-yellow/50 rounded-lg p-4">
      <div className="flex items-start gap-2 mb-3">
        <AlertCircle className="w-5 h-5 text-accent-yellow flex-shrink-0 mt-0.5" />
        <p className="text-sm text-accent-yellow">{clarification.question}</p>
      </div>
      ...
      {clarification.options && clarification.options.length > 0 ? (
        <div className="space-y-2">
          {clarification.options.map((option) => (
            <button
              ...
              className="w-full text-left px-3 py-2 rounded-lg bg-dark-card border border-border-default hover:border-accent-purple hover:bg-dark-elevated transition-colors"
            >
        ...
        <div className="flex gap-2">
          <input
            ...
            className="flex-1 px-3 py-2 text-sm border border-border-default rounded-lg bg-dark-card text-text-primary placeholder-text-muted focus:outline-none focus:ring-2 focus:ring-accent-purple"
          />
          <button
            onClick={() => {
              if (customInput.trim()) {
            ...
            className="px-3 py-2 bg-accent-purple text-white rounded-lg hover:bg-accent-purple/80 disabled:opacity-50 disabled:cursor-not-allowed"
          >
...
function StagedChangesView({
  ...
  return (
    <div className="border border-border-default rounded-lg overflow-hidden">
      <div className="bg-dark-card px-4 py-2 flex items-center justify-between">
        ...
        {pendingChanges.length > 0 && (
          <button
            ...
            className="text-xs px-2 py-1 bg-accent-green text-white rounded hover:bg-accent-green/80"
          >
            Approve All
          </button>
        )}
      ...
      <div className="divide-y divide-border-default">
        {changes.map((change) => (
          <div key={change.id} className="bg-dark-bg">
            <div
              className="px-4 py-2 flex items-center justify-between cursor-pointer hover:bg-dark-card"
            ...
            >
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-mono text-text-secondary">{change.filePath}</span>
              </div>
              <div className="flex items-center gap-2">
                {change.status === 'pending' && (
                  <>
                    ...
                    <button
                      ...
                      className="p-1 text-accent-red hover:bg-accent-red/20 rounded"
                    >
                      <X className="w-4 h-4" />
                    </button>
                  </>
            ...
            {expanded === change.id && (
              <div className="px-4 py-2 bg-dark-card">
                <pre className="text-xs font-mono overflow-x-auto p-2 bg-dark-canvas text-text-secondary rounded border border-border-default">
                  {change.newContent}
                </pre>
              </div>
            )}
    ...
    <div className="bg-dark-card border border-border-default rounded-lg overflow-hidden">
      ...
      <div className="px-4 py-2 flex items-center justify-between border-b border-border-default">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-medium text-text-primary">Browser Exploration</span>
        </div>
        <div className="flex items-center gap-2">
          ...
          {onClose && exploration.status === 'completed' && (
            <button
              onClick={onClose}
              className="p-1 text-text-muted hover:text-text-primary hover:bg-dark-elevated rounded"
            >
              <X className="w-3 h-3" />
            </button>
          )}
      ...
      {/* URL */}
      {exploration.url && (
        <div className="px-4 py-1.5 text-xs text-text-muted bg-dark-bg font-mono truncate">
          {exploration.url}
        </div>
      )}
      ...
      {exploration.screenshot && (
        <div className="p-2">
          <div className="relative rounded-lg overflow-hidden border border-border-default bg-dark-bg">
            <img
            ...
            {exploration.status !== 'completed' && exploration.status !== 'failed' && (
              <div className="absolute inset-0 bg-black/30 flex items-center justify-center">
                <div className="bg-dark-card/90 rounded-lg px-3 py-2 flex items-center gap-2">
                  <Loader2 className="w-4 h-4 animate-spin text-accent-purple" />
                  <span className="text-sm text-text-primary">{exploration.message || 'Processing...'}</span>
                </div>
              </div>
            )}
          </div>
        ...
        <div className="px-4 py-2 border-t border-border-default">
          ...
          <div className="flex flex-wrap gap-1.5">
            {exploration.discoveredElements.slice(0, 6).map((el, i) => (
              <span
                key={i}
                className="inline-flex items-center px-2 py-0.5 rounded-full text-xs bg-accent-purple/20 text-accent-purple"
                title={el.selector}
              >
            ...
            {exploration.discoveredElements.length > 6 && (
              <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs bg-dark-elevated text-text-muted">
...
export function CopilotPanel({ projectId, onCodeGenerated, className = '' }: CopilotPanelProps) {
  ...
  return (
    <div className={`flex flex-col h-full bg-dark-bg ${className}`}>
      {/* Header */}
      <div className="flex items-center justify-between px-4 py-3 border-b border-border-default">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h2 className="font-semibold text-sm text-text-primary">Vero Copilot</h2>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {messages.length === 0 && (
          <div className="text-center py-8">
            <div className="w-16 h-16 mx-auto mb-4 rounded-full bg-gradient-to-br from-accent-purple to-accent-pink flex items-center justify-center">
            ...
            <div className="mt-4 space-y-2">
              <p className="text-xs text-text-muted">Try saying:</p>
              <div className="flex flex-wrap gap-2 justify-center">
                {[
                  ...
                  <button
                    ...
                    className="text-xs px-3 py-1.5 bg-dark-card text-text-secondary rounded-full hover:bg-dark-elevated"
                  >
                    {suggestion}
                  </button>
                ))}
      ...
      <div className="p-4 border-t border-border-default">
        <div className="flex gap-2">
          <input
            ...
            className="flex-1 px-4 py-2 border border-border-default rounded-lg bg-dark-card text-text-primary placeholder-text-muted focus:outline-none focus:ring-2 focus:ring-accent-purple focus:border-transparent disabled:opacity-50 disabled:cursor-not-allowed"
          />
          <button
            onClick={sendMessage}
            disabled={
            ...
            className="px-4 py-2 bg-accent-purple text-white rounded-lg hover:bg-accent-purple/80 disabled:opacity-50 disabled:cursor-not-allowed"
          >

import { ElementInfo } from './pageObjectRegistry';
import { CapturedElement } from './selectorHealing';
...
/**
 * Strip modifier clauses (WITH TEXT, FIRST, LAST, NTH, HAS, WITHOUT TEXT, HAS NOT)
 * from a Vero selector so that only the base selector is stored in page objects.
 * Modifiers are runtime refinements and don't belong in FIELD definitions.
 */
export function stripSelectorModifiers(selector: string): string {
    return selector
        .replace(/\s+WITH(OUT)?\s+TEXT\s+"[^"]*"/gi, '')
        .replace(/\s+HAS\s+(NOT\s+)?(?:role|text|label|placeholder|alt|title|testid|css|xpath)\s+"[^"]*"/gi, '')
        .replace(/\s+(FIRST|LAST)\b/gi, '')
        .replace(/\s+NTH\s+\d+/gi, '')
        .trim();
}
 ...
 */
export function selectorToElementInfo(selector: string): ElementInfo {
    const element: ElementInfo = {
        tagName: 'div', // Default tag
    };

    // testid "login-btn"
    let match = selector.match(/testid "(.+?)"/i);
    if (match) {
    ...
    match = selector.match(/^role "(.+?)"(?: name "(.+?)")?/);
    if (match) {
    ...
    // button "Submit" or link "Click here" legacy selector
 ...
 */
export function selectorToCapturedElement(selector: string): CapturedElement {
